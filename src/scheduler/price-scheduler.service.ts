...
import {
  CredentialStatus,
  PortfolioCredential,
} from 'src/entities/portfolio-credential.entity';
...
import { ExchangeType as DomainExchange, Pair } from 'src/shared/domain/pair';
...
export class PriceSchedulerService {
  ...
  async collectPriceData() {
    ...
    try {
      ...
      for (const [exchangeId, credentials] of exchangeGroups) {
        const result = await this.collectDataForExchange(
          exchangeId,
          credentials,
        );
      ...
      this.logger.log(
        `스케줄러 완료: ${totalCollected}개 수집, ${totalErrors}개 실패, ${duration}ms 소요`,
      );
    } catch (error) {
    ...
    try {
      ...
      for (const portfolio of activePortfolios) {
        try {
          ...
          this.logger.error(
            `Failed to create snapshot for portfolio ${portfolio.id}:`,
            error,
          );
        }
      }
      ...
      this.logger.log(
        `Created snapshots for ${activePortfolios.length} portfolios`,
      );
    } catch (error) {
  ...
  private groupCredentialsByExchange(
    credentials: PortfolioCredential[],
  ): Map<string, PortfolioCredential[]> {
  ...
  private async collectDataForExchange(
    ...
    try {
      ...
      if (priceRecords.length > 0) {
        ...
        this.logger.log(
          `[${exchangeName}]${priceRecords.length}개 가격 데이터 저장 완료 `,
        );
      }
  ...
  async getSchedulerStatus() {
    ...
    return {
      ...
      connectedExchanges: [
        ...new Set(activeCredentials.map((c) => c.exchange.name)),
      ],
