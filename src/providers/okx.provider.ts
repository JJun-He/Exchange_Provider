...
export class OkxProvider extends BaseProvider{
    ...
    constructor(httpService: HttpService, apiKey?: string, secretKey?: string, passphrase?: string) {
   ...
   // OKX 에러 처리
    protected handleError(error: any): Error {
        if (error.response?.data) {
            ...
            return new Error(`OKX API Error: ${okxError.msg} (Code: ${okxError.code})`);
        }
        return new Error(`OKX API Request Failed: ${error.message}`);
    }

    // 입금 내역 조회
    async getDepositHistory(ccy?: string, state: number = 2, after?: number, before?: number) {
    ...
    // 출금 내역 조회
    async getWithdrawalHistory(ccy?: string, state: number = 2, after?: number, before?: number) {
        const params: any = { state };
        if (ccy) params.ccy = ccy;
        if (after) params.after = after;
        if (before) params.before = before;
